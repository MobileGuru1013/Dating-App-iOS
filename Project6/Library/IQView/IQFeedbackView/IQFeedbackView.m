...
-(id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
		[self setFrame:CGRectMake(10, -204, kWidth-20, 204)];
        ...
        if(!IOS_7_OR_GREATER){
            textViewFeedback = [[DALinedTextView alloc] initWithFrame:CGRectMake(5, 49, self.frame.size.width-10, 150)];
            
        }
        else{
            textViewFeedback = [[UITextView alloc] initWithFrame:CGRectMake(5, 49, self.frame.size.width-10, 150)];
...
-(void)setCanAddImage:(BOOL)canAddImage
{
	...
	if (_canAddImage == YES)
	{
		[textViewFeedback setFrame:CGRectMake(textFrame.origin.x, textFrame.origin.y, self.frame.size.width-10, textFrame.size.height)];
