...
//    cell.imageView.image = photo.thumbnailImage;
    [cell.imageView setImageWithURL:[NSURL URLWithString:photo.imageSTR]];
    return cell;
}
...
//    [self.imageScrollView displayImage:photo.originalImage];
    [self.imageScrollView displayImageWithURL:photo.imageSTR];
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        ...
        if (!error) {
            ...
            for(PFObject *object in objects) {
                TWPhoto *photo = [[TWPhoto alloc] init];
                photo.imageSTR = object[PF_PHOTO_PICTURE];
                [array_photo addObject:photo];
            }
            self.allPhotos = [NSArray arrayWithArray:array_photo];

            TWPhoto *firstPhoto = [self.allPhotos objectAtIndex:0];
            
            [self.imageScrollView displayImageWithURL:firstPhoto.imageSTR];
