...
- (id)initWithDelegate:(id <MWPhotoBrowserDelegate>)delegate {
    if ((self = [self init])) {
        _delegate = delegate;
        [[UINavigationBar appearance] setBarTintColor:[UIColor colorWithRed:184.0/255.0 green:81.0/255.0 blue:179.0/255.0 alpha:1.0]];
	}
	return self;
}
...
//	_pagingScrollView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    ...
    // Toolbar
    _toolbar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 100, 320, 300)];
...
//    _toolbar.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleWidth;
    ...
    if(_enableSwipeNextPrev) {
        UISwipeGestureRecognizer *swipeGes = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeNextPrevClicked:)];
...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"OK", nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
        ...
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Done", nil) style:UIBarButtonItemStylePlain target:self action:@selector(privateDoneButtonPressed:)];
...
//    _doneButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Done", nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
    ...
    if (_actionButton && !(!hasItems && !self.navigationItem.rightBarButtonItem)) {
        ...
        if(_enableDeleteBtn)
            [items insertObject:[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemTrash target:self action:@selector(deleteBtnClicked)] atIndex:0];
    } else {
...
- (void)privateDoneButtonPressed:(id) sender {

    if(self.delegate && [self.delegate respondsToSelector:@selector(privateDoneBtnClicked:)]) {
        [self.delegate privateDoneBtnClicked:self];
    } else {
...
- (void)restorePreviousNavBarAppearance:(BOOL)animated {
    if (_didSavePreviousStateOfNavBar) {
        ...
        // Restore back button if we need to // Valon
        
        if (_previousViewControllerBackButton) {
            UIViewController *previousViewController = [self.navigationController topViewController]; // We've disappeared so previous is now top
            previousViewController.navigationItem.backBarButtonItem = _previousViewControllerBackButton;
            previousViewController.navigationItem.backBarButtonItem.tintColor = [UIColor whiteColor];
...
//	_toolbar.frame = [self frameForToolbarAtOrientation:self.interfaceOrientation];
    _toolbar.frame = CGRectMake(0, 508, 375, 44); // tooljesper
        self.selectButton.frame = CGRectMake(0, 200, 100, 100);
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            ...
            if (self.displaySelectionButtons) {
                ...
                [self.selectButton setImage:[UIImage imageNamed:@"MWPhotoBrowser.bundle/images/ImageSelectedOff.png"] forState:UIControlStateNormal];
                [self.selectButton setImage:[UIImage imageNamed:@"MWPhotoBrowser.bundle/images/ImageSelectedOn.png"] forState:UIControlStateSelected];
                [self.selectButton sizeToFit];
                self.selectButton.adjustsImageWhenHighlighted = NO;
                [self.selectButton addTarget:self action:@selector(selectedButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
                self.selectButton.frame = [self frameForSelectedButton:self.selectButton atIndex:index];
                [_pagingScrollView addSubview:self.selectButton];
                page.selectedButton = self.selectButton;
                self.selectButton.selected = [self photoIsSelectedAtIndex:index];
            }
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            self.nabTitle = [[UILabel alloc] initWithFrame:CGRectMake(self.view.width/2-80, 8, 160, 30)];
            self.nabTitle.textAlignment = NSTextAlignmentCenter;
            self.nabTitle.textColor = [UIColor whiteColor];
            self.nabTitle.text = NSLocalizedString(@"Select Photos", nil);
        } else {
            NSString *photosText;
            if (numberOfPhotos == 1) {
            ...
            self.nabTitle.text = [NSString stringWithFormat:@"%lu %@", (unsigned long)numberOfPhotos, photosText];
        }
    } else if (numberOfPhotos > 1) {
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
            self.nabTitle.text = [_delegate photoBrowser:self titleForPhotoAtIndex:_currentPageIndex];
        } else {
            self.nabTitle.text = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)numberOfPhotos];
        }
	} else {
		self.nabTitle.text = nil;
	}
    ...
    self.navigationController.navigationBarHidden = YES;
    UIView *nav = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 375, 44)];
    nav.backgroundColor = [UIColor colorWithRed:151.0/255.0 green:79.0/255.0 blue:181.0/255.0 alpha:1.0];
    
    // Back button
    
    UIButton *btnBack = [[UIButton alloc] initWithFrame:CGRectMake(0, 7, 130, 32)];
    [btnBack setTitle:@"Create Test" forState:UIControlStateNormal];
    [btnBack.titleLabel setFont:[UIFont systemFontOfSize:17]];
    [btnBack addTarget:self action:@selector(BtnBackClicked) forControlEvents:UIControlEventTouchUpInside];
    [btnBack setTitleColor:[UIColor whiteColor] forState:UIControlStateHighlighted];
    [nav addSubview:btnBack];
    
    UIButton *btnDone = [[UIButton alloc] initWithFrame:CGRectMake(270, 7, 130, 32)];
    [btnDone setTitle:@"Done" forState:UIControlStateNormal];
    [btnDone.titleLabel setFont:[UIFont systemFontOfSize:17]];
    ...
    [nav addSubview:btnDone];

    // Arrow Image
    
    UIImageView *arrow = [[UIImageView alloc] initWithFrame:CGRectMake(0, 9.5, 25, 25)];
    arrow.image = [UIImage imageNamed:@"bakArrow.png"];
    [nav addSubview:arrow];
    
    // Title
    
    [nav addSubview:self.nabTitle];
...
- (void)btnDone
{
    ...
    int count = 0;
    for(NSNumber *num in appDelegate.selectArry) {
        if([num boolValue]) {
            count++;
        }
    }
    ...
    if(count >1) {
        ...
        for(int i=0; i<appDelegate.photoArray.count; i++) {
            
            NSNumber *photo_val = [appDelegate.selectArry objectAtIndex:i];
            if([photo_val boolValue]) {
                
                [array_test addObject:[appDelegate.photoArray objectAtIndex:i]];
            }
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    if (slideAndFade && [self areControlsHidden] && !hidden && animated) {
        ...
        // Toolbar
//        _toolbar.frame = CGRectOffset([self frameForToolbarAtOrientation:self.interfaceOrientation], 0, animatonOffset); // valon
...
- (void)swipeNextPrevClicked:(id)sender {
